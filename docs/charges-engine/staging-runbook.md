...
### 5b.2b History is not backfilled

The endpoint that priced a user's existing transactions was removed with ADR-32 — the cutover
applies forward only, so re-driving history is something nothing should ever do. Trades made before
shadow recording was switched on therefore carry no computed charge, and the reconciliation report
counts them under `transactionsWithoutComputation` rather than pricing them.

The figures in §5b.5b below were produced while that endpoint still existed. They are kept because
they are the evidence behind ADR-31, not because the run can be repeated.
